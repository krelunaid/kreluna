...
export default function RisonixControlPage() {
  ...
  async function mutate(id: string, action: "release-device" | "disable" | "delete") {
    if (action === "disable" && !window.confirm("Disattivare definitivamente questa licenza?")) return;
    if (action === "delete" && !window.confirm("Eliminare definitivamente questa licenza disattivata? Verranno rimossi anche il dispositivo associato e la cronologia tecnica. L’operazione non può essere annullata.")) return;
    ...
    setMessage(response.ok ? (action === "delete" ? "Licenza eliminata definitivamente." : action === "disable" ? "Licenza disattivata." : "Dispositivo sospeso. La licenza resta vincolata allo stesso computer.") : body?.error ?? "Operazione non riuscita.");
  ...
  async function deleteOrder(orderId: string) {
    if (!window.confirm("Annullare il checkout Stripe ed eliminare questo ordine non pagato? L’operazione non può essere annullata.")) return;
    const response = await fetch(`/api/risonix/orders/${orderId}/delete`, { method: "POST" });
    const body = await response.json() as { error?: string };
    setMessage(response.ok ? "Checkout annullato e ordine eliminato." : body.error ?? "Eliminazione non riuscita.");
    if (response.ok) await load();
  }
