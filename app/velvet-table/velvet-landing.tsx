...
export default function VelvetLanding({ locale }: { locale: VelvetLocale }) {
  ...
  const structuredData = {
    ...
    "@graph": [
      ...
      { "@type": "WebPage", "@id": `${pageUrl}#webpage`, url: pageUrl, name: copy.title, description, inLanguage: copy.locale, isPartOf: { "@id": `${siteUrl}/#website` }, about: { "@id": `${pageUrl}#service` }, primaryImageOfPage: { "@type": "ImageObject", url: `${siteUrl}/velvet-table/hero.jpg`, width: 1600, height: 1067 }, dateModified: "2026-09-06" },
      { "@type": "Service", "@id": `${pageUrl}#service`, name: "Velvet Table", alternateName: "Velvet Table by Kreluna", serviceType: copy.serviceType, description: copy.disclosure, provider: { "@id": `${siteUrl}/#organization` }, url: pageUrl, availableLanguage: Object.values(velvetCopy).map((item) => item.locale), audience: { "@type": "Audience", audienceType: copy.audience } },
    ...
    <div className="velvet-page" lang={copy.locale}>
      ...
      <main id="velvet-main">
        <section className="velvet-page-hero section-shell">
          ...
          <div className="velvet-page-hero-copy"><div className="eyebrow velvet-text"><i /> {copy.kicker}</div><h1>{copy.hero[0]}<br /><em>{copy.hero[1]}</em></h1><p>{copy.lead}</p><div className="hero-actions"><a className="button velvet-page-button" href="#come-funziona">{copy.discover}</a><Link className="button button-secondary" href={locale === "it" ? "/" : `/${locale}`}>{copy.back}</Link></div><p className="velvet-disclosure">{copy.disclosure}</p></div>
          <figure className="velvet-app-hero">{/* eslint-disable-next-line @next/next/no-img-element */}<img src="/velvet-table/hero-1200.webp" srcSet="/velvet-table/hero-640.webp 640w, /velvet-table/hero-1200.webp 1200w" sizes="(max-width: 900px) 90vw, 52vw" decoding="async" width="1600" height="1067" alt={copy.heroAlt} fetchPriority="high" /><figcaption><span>01</span><strong>{copy.heroCaption[0]}</strong><small>{copy.heroCaption[1]}</small></figcaption></figure>
        </section>
      ...
      <footer className="velvet-page-footer"><Link href={projectHubPath[locale]}>Kreluna</Link><span>{copy.footer}</span><a href={locale === "it" ? "/velvet-table/privacy" : "/en/privacy"}>Privacy</a></footer>
