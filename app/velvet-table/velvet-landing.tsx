...
export default function VelvetLanding({ locale }: { locale: VelvetLocale }) {
  ...
  const structuredData = {
    ...
    "@graph": [
      { "@type": "WebPage", "@id": `${pageUrl}#webpage`, url: pageUrl, name: copy.title, description: copy.description, inLanguage: copy.locale, isPartOf: { "@id": `${siteUrl}/#website` }, about: { "@id": `${pageUrl}#service` }, primaryImageOfPage: { "@type": "ImageObject", url: `${siteUrl}/velvet-table/hero.jpg`, width: 1600, height: 1067 }, dateModified: "2026-08-23" },
      { "@type": "Service", "@id": `${pageUrl}#service`, name: "Velvet Table", alternateName: "Velvet Table by Kreluna", serviceType: copy.serviceType, description: copy.description, provider: { "@id": `${siteUrl}/#organization` }, url: pageUrl, areaServed: "Worldwide", availableLanguage: Object.values(velvetCopy).map((item) => item.locale), audience: { "@type": "Audience", audienceType: copy.audience }, additionalProperty: { "@type": "PropertyValue", name: "Created with", value: "Helix" } },
      ...
      { "@type": "FAQPage", "@id": `${pageUrl}#faq`, inLanguage: copy.locale, mainEntity: copy.faqs.map((faq) => ({ "@type": "Question", name: faq.question, acceptedAnswer: { "@type": "Answer", text: faq.answer } })) },
    ],
  };
    ...
    <div className="velvet-page" lang={copy.locale}>
      ...
      <header className="velvet-page-header">
        ...
        <div className="velvet-header-actions">
          ...
          <a className="button button-small velvet-page-button" href="mailto:krelunaid@gmail.com?subject=Velvet%20Table">{copy.follow}</a>
        </div>
      </header>
      <main id="velvet-main">
