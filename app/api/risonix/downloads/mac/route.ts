...
import { installerBucket, RISONIX_MAC_INSTALLER_KEY } from "@/app/risonix/installer-storage";
...
export async function GET() {
  ...
  const installer = await installerBucket().get(RISONIX_MAC_INSTALLER_KEY);
  if (!installer) return new Response("Installer temporaneamente non disponibile", { status: 503 });

  const headers = new Headers({
    "content-type": "application/x-apple-diskimage",
    "content-disposition": 'attachment; filename="Risonix-1.0.0-macOS-arm64.dmg"',
