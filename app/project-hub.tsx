...
export default function ProjectHub({ locale }: { locale: ProjectHubLocale }) {
  const t = copy[locale];
  const canonical = `${siteUrl}${projectHubPaths[locale]}`;
  const contactHref = locale === "it" ? "/contatti.html" : locale === "en" ? "/en/contact.html" : `mailto:andrea@kreluna.it?subject=${encodeURIComponent(`Kreluna ${locale.toUpperCase()}`)}`;
  const projects = [
    { name: "CityBeam", status: t.cityStatus, tagline: t.cityTagline, href: cityBeamPaths[locale], image: "/citybeam-hero.png", slug: "citybeam" },
    { name: "Velvet Table", status: t.velvetStatus, tagline: t.velvetTagline, href: velvetPaths[locale], image: "/velvet-table/hero.jpg", slug: "velvet-table" },
  ];
  ...
  return (
    <div className="projects-page" lang={t.lang}>
      <header className="projects-page-header">
        <Link className="projects-page-brand" href={projectHubPaths[locale]} aria-label="Kreluna projects"><img src="/kreluna-logo.png" alt="" width="34" height="34" /><span>KRELUNA</span></Link>
        <nav aria-label="Projects navigation"><Link href={projectHubPaths[locale]}>{t.home}</Link><a href={cityBeamPaths[locale]}>CityBeam</a><a className="button button-small button-primary" href={contactHref}>{t.contact}</a></nav>
      </header>
      <main>
